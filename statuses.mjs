export const statusDefinitions = {
 ...
 blind:{name:'Ослепление',kind:'malus',icon:'◒'},
...
export function renderStatuses(container,actor){
 container.replaceChildren(...activeStatuses(actor).map(status=>{
  ...
  const img=status.image?document.createElement('img'):document.createElement('span');
  if(status.image){img.src=status.image;img.alt='';}
  else {img.className='status-glyph';img.textContent=status.icon;img.setAttribute('aria-hidden','true');}
