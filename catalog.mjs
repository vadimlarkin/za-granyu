...
export function describe(c){
 ...
 const special={none:'',rob:`Забрать до ${x} случайных карт из руки противника в свою руку.`,confuse:`Оглушение ${x}: следующий добор противника меньше на ${x}.`,cheap:'Скидка уже включена в стоимость.',draw:`Добрать ${x} карт.`,haste:`Снизить стоимость текущей руки на ${x}, минимум до 0.`,surprise:`Дополнительно ${x} урона оглушённому противнику.`,fortify:'При действующем уклонении получить оборонительную стойку: остаток физической защиты сохраняется до конца боя.'}[c.special??'none'];
 return [primary,special,c.bonus==='vigor'?`Бодрость духа ${c.bonusAmount}: следующий добор больше на ${c.bonusAmount}.`:c.bonus==='stance'?'Оборонительная стойка: сохранять физическую защиту до конца боя.':'',c.malus==='stun'?`Оглушение ${c.malusAmount}: следующий добор противника меньше на ${c.malusAmount}.`:''].filter(Boolean).join(' ');
}
