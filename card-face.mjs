import {effectAmount,SPECIALS,BONUSES,MALUSES,specialAmount} from './catalog.mjs?v=20260922-street-cards-1';
...
const compactSpecials={rob:'Грабёж',draw:'Добор',haste:'Скидка',surprise:'Сюрприз',fortify:'Укрепление',leftHook:'Комбо',combo:'Добор',jab:'Джеб',series:'Серия'};
...
for(const id of ['left-hook','clever-feint','tight-guard','brass-knuckles','best-defense','sucker-punch','stash','spit','sidestep','poke'])uniqueArt.add(id);
...
export function compactCardLabels(card,{amount=effectAmount(card),special=specialAmount(card)}={}){
 ...
 if(card.malus==='blind')result.push(`Ослепление ${card.malusAmount}`);
...
export function cardFace(card,{amount=effectAmount(card),special=specialAmount(card),state='',discount=false,compact=false}={}){
 ...
 const compactText=compactCardLabels(card,{amount,special}).map(label=>{
  const safe=escape(label).replace(/ ([+−-]?\d+)$/,` <span class="card-chip-number">$1</span>`);
  return `<span class="card-chip">${safe}</span>`;
 }).join('');
 ...
 const footer=`<span class="card-footer"><span>${escape(card.kind)}</span></span>`;
 return `<span class="card-art" aria-hidden="true"></span><span class="card-shell" aria-hidden="true"></span><span class="card-top"><span class="cost">${card.cost}${discount?'<small class="discount" aria-label="Скидка">↓</small>':''}</span><span class="card-color" role="img" aria-label="${colors[card.color]}">${symbols[card.color]}</span></span><strong class="card-name">${escape(card.name)}</strong><span class="card-text${compact?' card-text-compact':''}">${compact?compactText:`<b class="card-value" data-digits="${digits.length}">${card.effect==='none'?'✦':value}</b><span>${card.effect==='none'?(card.special==='draw'?'Добрать '+x+' карт':card.special==='rob'?'Забрать '+x+' карт':card.special==='haste'?'Стоимость руки −'+x:card.bonus==='vigor'?'Бодрость духа':card.malus==='stun'?'Оглушение':card.malus==='blind'?'Ослепление':'Без эффекта'):labels[card.effect]}<small>${card.effect==='none'&&card.special==='rob'?'из руки врага':detail}</small></span>`}</span><span class="card-special">${compact?'':escape(specialText)}</span>${footer}<span class="card-state">${escape(state)}</span>`;
}
